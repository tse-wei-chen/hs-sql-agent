    ...
    <template v-else>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            IP limits protect pre-auth traffic; key and concurrency limits protect authenticated SQL workloads.
          </CardDescription>
        </CardHeader>
        <CardContent class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <Field>
            <FieldLabel>IP requests per window</FieldLabel>
            <Input v-model.number="policy.ipPermitLimit" type="number" min="1" />
          </Field>
          <Field>
            <FieldLabel>IP window (seconds)</FieldLabel>
            <Input v-model.number="policy.ipWindowSeconds" type="number" min="1" />
          </Field>
          <Field>
          ...
          <div class="text-xs text-muted-foreground md:col-span-2 xl:col-span-3">
            IP quotas use the server connection address. Users behind NAT share one quota.
            Behind a reverse proxy, configure ASP.NET Core Forwarded Headers with explicit
            trusted proxies/networks before relying on the original client IP.
          </div>
        </CardContent>
      </Card>
