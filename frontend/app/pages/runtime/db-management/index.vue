...
const providerOptions = ['Sqlite', 'Postgres', 'MySQL', 'MsSqlServer', 'Oracle', 'Firebird']
// Form state
const form = ref({
    name: '',
    sqlProvider: 'Mysql',
    ...
    <div class="space-y-6">
        ...
        <Card>
            <CardHeader class="border-b bg-muted/40">
                <CardTitle>{{ editingId ? 'Edit Database Connection' : 'Create Database Connection' }}</CardTitle>
                <CardDescription>
                ...
                <form class="space-y-6" @submit.prevent="save">
                    <FieldGroup class="grid gap-4 md:grid-cols-2">
                        <Field class="md:col-span-2">
                            ...
                            <Input id="name" v-model="form.name" placeholder="e.g., Production DB" required />
                        </Field>
                        ...
                        <Field
                            v-if="['Postgres', 'MySQL', 'MsSqlServer', 'Oracle', 'Firebird'].includes(form.sqlProvider)">
                            <FieldLabel for="host">Host</FieldLabel>
                            <Input id="host" v-model="form.host" placeholder="e.g., localhost or 192.168.1.100" />
                        </Field>
                        ...
                        <Field
                            ...
                            <Input id="username" v-model="form.username" placeholder="Database user" />
                        </Field>
                        ...
                        <Field
                            ...
                            <p v-if="editingId" class="text-[0.7rem] text-muted-foreground mt-1">Leave blank to keep
                                existing password intact.</p>
                        </Field>

                        <Field
                            v-if="['Sqlite', 'Postgres', 'MySQL', 'MsSqlServer', 'Oracle', 'Firebird'].includes(form.sqlProvider)">
                            <FieldLabel for="database">Database</FieldLabel>
                            <Input id="database" v-model="form.database" placeholder="e.g., my_app_db" />
                        </Field>
                    </FieldGroup>
        ...
        <Card>
            ...
            <CardContent class="pt-6">
                <div v-if="loading" class="py-8 text-sm text-muted-foreground text-center">Loading connections...</div>
                <div v-else-if="dbs.length === 0" class="py-8 text-sm text-muted-foreground text-center">
                    No database connections defined yet.
                </div>
                <div v-else class="grid gap-4 md:grid-cols-2">
                    <div v-for="db in dbs" :key="db.id"
                        ...
                        <div class="flex items-start justify-between mb-2">
                            <div>
                                ...
                                <p class="text-xs text-muted-foreground mt-1">
                                    {{ db.host ? `${db.host}${db.port ? ':' + db.port : ''}` : 'No host defined' }}
                                    <span v-if="db.database"> | {{ db.database }}</span>
                                </p>
                            </div>
                            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <Button variant="ghost" size="icon" class="h-8 w-8" @click="startEdit(db)">
                                    <Edit2 class="size-4" />
                                </Button>
                        ...
                        <div
                            ...
                            <span>Updated: {{ new Date(db.updatedAt || db.createdAt).toLocaleString() }}</span>
                        </div>
                    </div>
                </div>
            </CardContent>
