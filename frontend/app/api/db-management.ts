...
export const createDbManagement = async (payload: DbManagementRequest): Promise<DbManagement> => {
  const response = await xiorInstanceToken.post('/DbManagement', payload)
  return response.data
}

export const updateDbManagement = async (id: number, payload: DbManagementRequest): Promise<void> => {
