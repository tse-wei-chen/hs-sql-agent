...
module internal RewriteParser =
    ...
    and private parseInTail (cursor: Cursor) (value: Expr) (negated: bool) : Expr =
        ...
        if acceptSymbol ')' cursor then
            fail cursor.Current "IN list cannot be empty"
    ...
    and private parseNonNegativeRowCount context (cursor: Cursor) =
        let token = cursor.Take()
        match token.Kind with
        | IntegerLiteral value when value <= int64 System.Int32.MaxValue ->
            NonNegativeRowCount.create (int value)
