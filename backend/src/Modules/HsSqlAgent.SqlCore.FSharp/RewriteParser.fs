...
module internal RewriteParser =
    ...
    let private fail (token: Token) message = invalidArg "sql" (message + " at offset " + string token.Start + ".")
    let private isKeyword keyword token = match token.Kind with | Keyword value -> value = keyword | _ -> false
    let private isSymbol symbol token = match token.Kind with | Symbol value -> value = symbol | _ -> false
    let private isOperator operator token = match token.Kind with | Operator value -> value = operator | _ -> false
    ...
    let private functionName identifier = identifier |> Identifier.parts |> List.map (fun part -> part.Value) |> String.concat "." |> FunctionName.create
    ...
    and private parseReturning cursor =
        if not (acceptKeyword "RETURNING" cursor) then [] else
    ...
    and private parseTableSource cursor =
        if acceptSymbol '(' cursor then
            ...
            expectSymbol ')' cursor
            let alias = if acceptKeyword "AS" cursor then identifierPart cursor else identifierPart cursor
    ...
    and private parseJoin cursor =
        ...
        let predicate = match kind with | JoinKind.Cross -> None | _ -> expectKeyword "ON" cursor; Some(parseExpression cursor)
    ...
    and private parseSelect cursor =
        ...
        { Distinct = distinct; Projection = projection |> Seq.toList; From = from; Joins = joins |> Seq.toList; Where = where; GroupBy = groupBy |> Seq.toList; Having = if acceptKeyword "HAVING" cursor then Some(parseExpression cursor) else None }
    and private parseOrderBy cursor =
        if not (acceptKeyword "ORDER" cursor) then [] else
    ...
    and private parseIntOption keyword cursor =
        if not (acceptKeyword keyword cursor) then None else
            let token = cursor.Take()
            match token.Kind with | IntegerLiteral value when value <= int64 System.Int32.MaxValue -> Some(int value) | _ -> fail token (keyword + " requires a non-negative integer")
    and private parseQuery cursor =
        let head = parseSelect cursor
        { Head = head; SetOperations = []; OrderBy = parseOrderBy cursor; Limit = parseIntOption "LIMIT" cursor; Offset = parseIntOption "OFFSET" cursor }
    and private parseInsert cursor =
        ...
        { Target = target; Columns = columns |> Seq.toList; Rows = rows |> Seq.toList; Source = source; Returning = parseReturning cursor }
    and private parseUpdate cursor =
        ...
        { Target = target; Assignments = assignments |> Seq.toList; Where = if acceptKeyword "WHERE" cursor then Some(parseExpression cursor) else None; Returning = parseReturning cursor }
    ...
    let parse (sql: string) =
        ...
        let statement = match cursor.Current.Kind with | Keyword "SELECT" -> Statement.QueryStatement(parseQuery cursor) | Keyword "INSERT" -> Statement.InsertStatement(parseInsert cursor) | Keyword "UPDATE" -> Statement.UpdateStatement(parseUpdate cursor) | Keyword "DELETE" -> Statement.DeleteStatement(parseDelete cursor) | _ -> fail cursor.Current "Expected SELECT, INSERT, UPDATE, or DELETE"
