...
module internal RewriteParser =
    ...
    and private parseComparison cursor =
        ...
        while keepGoing do
            let binary op = cursor.Advance(); left <- Expr.Binary(op, left, parseConcat cursor)
    ...
    and private parseInTail cursor value negated =
        ...
        if isKeyword "SELECT" cursor.Current then
            invalidOp "IN (subquery) requires the dedicated rewrite DU case before it can be accepted."
        else
    ...
    and private parseSelect cursor =
        ...
        { Distinct = distinct; Projection = projection |> Seq.toList; From = from; Joins = joins |> Seq.toList; Where = where; GroupBy = groupBy |> Seq.toList; Having = having }
