...
module internal RewriteRenderer =
    ...
    type RenderedCommand = { Sql: string; Parameters: (obj | null) list }
    ...
    let private quotePart provider (part: IdentifierPart) =
        let raw = if part.WasQuoted then part.Value else match provider with Oracle | Firebird -> part.Value.ToUpperInvariant() | _ -> part.Value
        match provider with
        | MySql -> "`" + raw.Replace("`", "``") + "`"
    ...
    let private unaryText = function UnaryOperator.Not -> "NOT" | UnaryOperator.Negate -> "-" | UnaryOperator.Positive -> "+"
    let private binaryText = function
        | BinaryOperator.Add -> "+" | BinaryOperator.Subtract -> "-" | BinaryOperator.Multiply -> "*" | BinaryOperator.Divide -> "/" | BinaryOperator.Modulo -> "%" | BinaryOperator.Concat -> "||"
        | BinaryOperator.Equal -> "=" | BinaryOperator.NotEqual -> "<>" | BinaryOperator.GreaterThan -> ">" | BinaryOperator.LessThan -> "<" | BinaryOperator.GreaterThanOrEqual -> ">=" | BinaryOperator.LessThanOrEqual -> "<="
        | BinaryOperator.Like -> "LIKE" | BinaryOperator.ILike -> "ILIKE" | BinaryOperator.And -> "AND" | BinaryOperator.Or -> "OR"
    let private joinText = function JoinKind.Inner -> "INNER JOIN" | JoinKind.Left -> "LEFT JOIN" | JoinKind.Right -> "RIGHT JOIN" | JoinKind.Full -> "FULL JOIN" | JoinKind.Cross -> "CROSS JOIN"
    let private setText = function SetOperator.Union -> "UNION" | SetOperator.UnionAll -> "UNION ALL" | SetOperator.Intersect -> "INTERSECT" | SetOperator.Except -> "EXCEPT"
    ...
    let rec private renderExpr (ctx: RenderContext) expression =
        ...
        | Expr.Interval literal -> match ctx.Provider with PostgreSql -> "CAST(" + ctx.Bind(box (IntervalLiteral.value literal)) + " AS interval)" | _ -> invalidOp "INTERVAL lowering is not available for this rewrite provider yet."
        | Expr.Unary(operator, operand) -> match operator with UnaryOperator.Not -> "NOT (" + renderExpr ctx operand + ")" | _ -> "(" + unaryText operator + renderExpr ctx operand + ")"
        | Expr.Binary(operator, left, right) ->
            let leftSql, rightSql = renderExpr ctx left, renderExpr ctx right
            ...
            | BinaryOperator.ILike, provider when provider <> PostgreSql -> invalidOp "ILIKE requires provider-specific lowering before rendering."
        ...
        | Expr.FilteredAggregate(value, predicate) -> match ctx.Provider with PostgreSql | SQLite | Firebird -> renderExpr ctx value + " FILTER (WHERE " + renderExpr ctx predicate + ")" | _ -> invalidOp "Aggregate FILTER requires provider lowering before rendering."
        ...
        | Expr.InList(value, items, negated) -> renderExpr ctx value + (if negated then " NOT IN " else " IN ") + "(" + (items |> NonEmpty.toList |> List.map (renderExpr ctx) |> String.concat ", ") + ")"
    ...
    and private renderWindow (ctx: RenderContext) window =
        ...
        if not window.OrderBy.IsEmpty then parts.Add("ORDER BY " + (window.OrderBy |> List.map (renderOrderBy ctx) |> String.concat ", "))
    ...
    and private renderOrderBy (ctx: RenderContext) order =
        let direction = if order.Descending then " DESC" else " ASC"
        let nulls = match order.NullOrdering with NullOrdering.Default -> "" | NullOrdering.NullsFirst -> match ctx.Provider with SqlServer | MySql -> invalidOp "NULLS FIRST requires lowering for this provider." | _ -> " NULLS FIRST" | NullOrdering.NullsLast -> match ctx.Provider with SqlServer | MySql -> invalidOp "NULLS LAST requires lowering for this provider." | _ -> " NULLS LAST"
        renderExpr ctx order.Expression + direction + nulls

    and private renderSource (ctx: RenderContext) source =
        match source with
        | TableSource.NamedTable(identifier, alias) -> renderIdentifier ctx.Provider identifier + (alias |> Option.map (fun value -> tableAliasPrefix ctx.Provider + renderAlias ctx.Provider value) |> Option.defaultValue "")
        | TableSource.DerivedTable(query, alias) -> "(" + renderQuery ctx query + ")" + tableAliasPrefix ctx.Provider + renderAlias ctx.Provider alias

    and private renderSelect (ctx: RenderContext) select =
        ...
        for join in select.Joins do sql <- sql + " " + joinText join.Kind + " " + renderSource ctx join.Source; join.Predicate |> Option.iter (fun predicate -> sql <- sql + " ON " + renderExpr ctx predicate)
    ...
    and private renderPaging (ctx: RenderContext) query sql =
        let bindCount value = ctx.Bind(box (NonNegativeRowCount.value value))
        match ctx.Provider with
        | PostgreSql | SQLite | MySql ->
            let withLimit = query.Limit |> Option.map (fun value -> sql + " LIMIT " + bindCount value) |> Option.defaultValue sql
            match query.Offset, ctx.Provider, query.Limit with Some value, MySql, None -> withLimit + " LIMIT 18446744073709551615 OFFSET " + bindCount value | Some value, _, _ -> withLimit + " OFFSET " + bindCount value | None, _, _ -> withLimit
        | SqlServer ->
            ...
            | Some offset, limit ->
                ...
                sql + " OFFSET " + bindCount offset + " ROWS" + (limit |> Option.map (fun value -> " FETCH NEXT " + bindCount value + " ROWS ONLY") |> Option.defaultValue "")
        | Oracle | Firebird ->
            let withOffset = query.Offset |> Option.map (fun value -> sql + " OFFSET " + bindCount value + " ROWS") |> Option.defaultValue sql
            query.Limit |> Option.map (fun value -> withOffset + " FETCH NEXT " + bindCount value + " ROWS ONLY") |> Option.defaultValue withOffset

    and private renderQueryBody (ctx: RenderContext) query =
        let mutable sql = renderSelect ctx query.Head
        for branch in query.SetOperations do sql <- sql + " " + setText branch.Operator + " " + renderQuery ctx branch.Query
        if not query.OrderBy.IsEmpty then sql <- sql + " ORDER BY " + (query.OrderBy |> List.map (renderOrderBy ctx) |> String.concat ", ")
        sql

    and private renderQuery (ctx: RenderContext) query =
        ...
        | SqlServer, None, Some limit, [] ->
            ...
            if sql.StartsWith("SELECT DISTINCT ", StringComparison.Ordinal) then "SELECT DISTINCT TOP (" + top + ") " + sql.Substring("SELECT DISTINCT ".Length) else "SELECT TOP (" + top + ") " + sql.Substring("SELECT ".Length)
        | Firebird, None, Some limit, [] ->
            let first = ctx.Bind(box (NonNegativeRowCount.value limit))
            let sql = renderQueryBody ctx { query with Limit = None }
            if sql.StartsWith("SELECT DISTINCT ", StringComparison.Ordinal) then "SELECT DISTINCT FIRST " + first + " " + sql.Substring("SELECT DISTINCT ".Length) else "SELECT FIRST " + first + " " + sql.Substring("SELECT ".Length)
        | _ -> renderPaging ctx query (renderQueryBody ctx query)

    let private renderReturning (ctx: RenderContext) items =
        if List.isEmpty items then ""
        else match ctx.Provider with PostgreSql | SQLite | Firebird -> " RETURNING " + (items |> List.map (fun item -> renderExpr ctx item.Expression + (item.Alias |> Option.map (fun alias -> " AS " + renderAlias ctx.Provider alias) |> Option.defaultValue "")) |> String.concat ", ") | _ -> invalidOp "RETURNING requires provider-specific lowering before rendering."
    ...
    let private renderInsert (ctx: RenderContext) insert =
        ...
        let sourceSql = match insert.Input with QuerySource query -> " " + renderQuery ctx query | Values rows -> " VALUES " + (rows |> NonEmpty.toList |> List.map (fun row -> "(" + (row |> NonEmpty.toList |> List.map (renderExpr ctx) |> String.concat ", ") + ")") |> String.concat ", ") | DefaultValues -> " DEFAULT VALUES"
    ...
    let render provider executable =
        ...
        let sql = match document.Statement with Statement.QueryStatement query -> renderQuery ctx query | Statement.InsertStatement insert -> renderInsert ctx insert | Statement.UpdateStatement update -> renderUpdate ctx update | Statement.DeleteStatement delete -> renderDelete ctx delete
