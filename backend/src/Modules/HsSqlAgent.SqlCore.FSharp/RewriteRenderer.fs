...
module internal RewriteRenderer =
    ...
    let private renderIdentifier (provider: Provider) (identifier: Identifier) =
        identifier |> Identifier.parts |> List.map (quotePart provider) |> String.concat "."

    let private renderAlias (provider: Provider) (alias: IdentifierPart) = quotePart provider alias
    ...
    let rec private renderExpr (ctx: RenderContext) (expression: Expr) : string =
        ...
        | Expr.SimpleCase(input, branches, fallback) ->
            let cases =
                branches
                |> NonEmpty.toList
                |> List.map (fun (branch: SimpleCaseBranch) -> " WHEN " + renderExpr ctx branch.Match + " THEN " + renderExpr ctx branch.Result)
                |> String.concat ""
        ...
        | Expr.SearchedCase(branches, fallback) ->
            let cases =
                branches
                |> NonEmpty.toList
                |> List.map (fun (branch: SearchedCaseBranch) -> " WHEN " + renderExpr ctx branch.Condition + " THEN " + renderExpr ctx branch.Result)
                |> String.concat ""
        ...
        | Expr.Between(value, lower, upper, negated) ->
            renderExpr ctx value + (if negated then " NOT BETWEEN " else " BETWEEN ") + renderExpr ctx lower + " AND " + renderExpr ctx upper
    ...
    and private renderWindow (ctx: RenderContext) (window: WindowSpec) : string =
        ...
        window.Frame
        |> Option.iter (fun (frame: WindowFrame) ->
            let unitText =
            ...
            let frameSql =
                ...
                | SingleBound start -> unitText + " " + frameBoundText start
                | BetweenBounds(start, finish) -> unitText + " BETWEEN " + frameBoundText start + " AND " + frameBoundText finish
            parts.Add(frameSql))
        String.concat " " parts
    ...
    and private renderOrderBy (ctx: RenderContext) (order: OrderBy) : string =
        ...
        let nulls =
            ...
            | NullOrdering.NullsFirst ->
                match ctx.Provider with
                | Provider.SqlServer | Provider.MySql -> invalidOp "NULLS FIRST requires lowering for this provider."
                | _ -> " NULLS FIRST"
            | NullOrdering.NullsLast ->
                match ctx.Provider with
                | Provider.SqlServer | Provider.MySql -> invalidOp "NULLS LAST requires lowering for this provider."
                | _ -> " NULLS LAST"
    ...
    and private renderSource (ctx: RenderContext) (source: TableSource) : string =
        ...
        | TableSource.NamedTable(identifier, alias) ->
            ...
            + (alias |> Option.map (fun value -> tableAliasPrefix ctx.Provider + renderAlias ctx.Provider value) |> Option.defaultValue "")
    ...
    and private renderPaging (ctx: RenderContext) (query: Query) (sql: string) : string =
        let bindCount (value: NonNegativeRowCount) = ctx.Bind(box (NonNegativeRowCount.value value))
        match ctx.Provider with
        | PostgreSql | SQLite | MySql ->
            ...
            | Some value, MySql, None -> withLimit + " LIMIT 18446744073709551615 OFFSET " + bindCount value
            | Some value, _, _ -> withLimit + " OFFSET " + bindCount value
            | None, _, _ -> withLimit
        | SqlServer ->
            ...
            | Some offset, limit ->
                ...
                sql + " OFFSET " + bindCount offset + " ROWS"
                + (limit |> Option.map (fun value -> " FETCH NEXT " + bindCount value + " ROWS ONLY") |> Option.defaultValue "")
        | Oracle | Firebird ->
    ...
    and private renderQuery (ctx: RenderContext) (query: Query) : string =
        ...
        | SqlServer, None, Some limit, [] ->
            ...
            else "SELECT TOP (" + top + ") " + sql.Substring("SELECT ".Length)
        | Firebird, None, Some limit, [] ->
            ...
            else "SELECT FIRST " + first + " " + sql.Substring("SELECT ".Length)
    ...
    let private renderInsert (ctx: RenderContext) (insert: Insert) : string =
        ...
        let sourceSql =
            ...
            | Values rows ->
                ...
                + (rows
                   |> NonEmpty.toList
                   |> List.map (fun row -> "(" + (row |> NonEmpty.toList |> List.map (renderExpr ctx) |> String.concat ", ") + ")")
                   |> String.concat ", ")
    ...
    let private renderUpdate (ctx: RenderContext) (update: Update) : string =
        let assignments =
            update.Assignments
            |> List.map (fun (assignment: Assignment) -> renderIdentifier ctx.Provider assignment.Target + " = " + renderExpr ctx assignment.Value)
            |> String.concat ", "
