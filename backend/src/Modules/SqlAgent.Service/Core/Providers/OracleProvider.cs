...
public class OracleProvider : SqlProviderBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            const string sql = @"
                SELECT
                    ac.CONSTRAINT_NAME AS KEY_NAME,
                    ...
                    0 AS HAS_EXPRESSIONS,
                    acc.POSITION AS KEY_ORDINAL,
                ...
                WHERE i.TABLE_OWNER = :schemaName
            ...
            return [.. rows
                .GroupBy(row => ((string)row.KEY_NAME, Convert.ToInt32(row.IS_PRIMARY_KEY) != 0))
                .Select(group =>
                {
