...
/// <summary>
/// Validates authorization and provider capabilities from canonical Core AST + QueryFacts.
/// No DTO traversal occurs here: table access is decided exclusively from binder-produced facts.
/// </summary>
public sealed class CoreSqlPlanValidator : ISqlPlanValidator
{
        ...
        IReadOnlySet<string>? allowedTables)
    {
        ...
        var allowed = allowedTables is HashSet<string> hash
            && hash.Comparer.Equals(StringComparer.OrdinalIgnoreCase)
            ? hash
    ...
    {
        if (provider is not (SqlAgentToolType.MySQL or SqlAgentToolType.MsSqlServer))
            return;
    ...
    {
        if (kind is "INNER" or "LEFT" or "RIGHT" or "FULL" or "CROSS")
            return;
