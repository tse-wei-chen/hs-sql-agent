    ...
    ISecurityPolicyRuntimeState runtimeState) : ISecurityPolicyService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        entity.IpPermitLimit = request.IpPermitLimit;
        entity.IpWindowSeconds = request.IpWindowSeconds;
    ...
    private static void Validate(SecurityPolicyModel request)
    {
        ...
        if (request.IpPermitLimit is < 1 or > 1_000_000)
            throw new ArgumentException("IpPermitLimit must be between 1 and 1000000.");
        if (request.IpWindowSeconds is < 1 or > 86_400)
            throw new ArgumentException("IpWindowSeconds must be between 1 and 86400.");
