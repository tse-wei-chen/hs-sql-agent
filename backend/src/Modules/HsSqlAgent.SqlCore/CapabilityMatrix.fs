            ...
            [
                ...
                cap("dml.upsert_merge","dml",upsertStatus,
                    if upsertStatus=translated then
                        if provider=SqlAgentToolType.Postgres then
                            "PostgreSQL supports the deterministic Core explicit-target conflict contract, including INSERT VALUES and assurance-backed INSERT SELECT. Direct target = EXCLUDED.source assignments retain the existing portable lowering path. Native PostgreSQL additionally admits a closed deterministic conflict-update scalar algebra over EXCLUDED columns, unqualified current-row target columns, parameterized literals, unary +/- and arithmetic +, -, *, /, %. Rich expressions remain native-only; functions, predicates, concatenation, casts, CASE, subqueries, named constraints, partial-index predicates, and typed approval execution remain fail-closed."
                        else
                            "SQLite ServerVersion 3.24+ target profiles support the deterministic Core INSERT VALUES conflict contract with an explicit conflict-column target. Direct target = EXCLUDED.source assignments retain the existing portable lowering path. Native SQLite additionally admits the closed deterministic conflict-update scalar algebra over EXCLUDED columns, unqualified current-row target columns, parameterized literals, unary +/- and arithmetic +, -, *, /, %. Targetless DO NOTHING remains native-only. The target version must be explicit; rich expressions are native-only and conflict predicates/functions/subqueries plus richer SQLite UPSERT grammar and typed approval execution remain fail-closed."
                    else
                        match provider with
                        | SqlAgentToolType.Sqlite ->
