...
public partial class SqlAgentTool
{
    ...
    {
        dbType = default;
        if (string.IsNullOrEmpty(sqlConfig.Provider) || string.IsNullOrEmpty(sqlConfig.ConnectionString)) return false;
    ...
    private Task<SqlRuntimeConfig> ResolveSqlConfigAsync()
    {
        ...
        if (context is not null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(provider) && !string.IsNullOrWhiteSpace(connectionString))
                return Task.FromResult(new SqlRuntimeConfig { Provider = provider, ConnectionString = connectionString });
        }
    ...
    private int? ResolveAccessKeyId()
    {
        var context = _httpContextAccessor.HttpContext;
        return context != null
               && context.Items.TryGetValue(McpContextItemKeys.AccessKeyId, out var idObj)
               && idObj is int id ? id : null;
    }
    ...
    private void EnsureTableAllowed(string tableName)
    {
        ...
        if (!whitelist.Contains(tableName))
            throw new UnauthorizedAccessException($"API key does not have permission to access table: {tableName}");
    }
}
