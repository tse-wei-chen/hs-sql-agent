...
public sealed class FirebirdGrammarMatrixTests
{
    ...
    [
        ...
        new("typed-date", new BodyVariant(
            ...
            "EVENTS", ["events"])),
        new("typed-time", new BodyVariant(
            "SELECT TIME '12:34:56' AS id FROM events",
            "EVENTS", ["events"])),
        new("typed-timestamp", new BodyVariant(
            "SELECT TIMESTAMP '2026-08-24 12:34:56' AS id FROM events",
            "EVENTS", ["events"]))
    ];
    ...
    [
        ...
        new("order", new TailVariant(
            " ORDER BY id", new CanonicalPagingExpectation(null, null))),
        ...
        new("fetch-first", new TailVariant(
            ...
            new CanonicalPagingExpectation(10, null))),
    ...
    [Fact]
    public void FirebirdCteGrammarMatrix_IsCartesianAndCollisionFree()
    {
        var cases = FirebirdCteGrammarMatrix().ToArray();
    ...
    [Theory]
    [MemberData(nameof(FirebirdCteGrammarMatrix))]
    public void FirebirdCteGrammarMatrix_ParsesBindsValidatesCompilesAndRenders(
        string name,
        string sql,
    ...
    {
        var parsed = CoreSqlTextParser.ParseQuery(sql, SqlAgentToolType.Firebird);
        ...
        var command = SqlCoreFacade.CompileQuery(
            ...
            SqlAgentToolType.Firebird,
            new SqlPlanValidationContext("firebird-combinatorial-grammar-matrix-v1"),
        ...
        if (name.Contains("__typed-time__", StringComparison.Ordinal))
        {
            Assert.DoesNotContain("12:34:56", command.Sql, StringComparison.Ordinal);
            Assert.Contains(
                command.Parameters,
                parameter => parameter.Value is TimeSpan value
                    && value == new TimeSpan(12, 34, 56));
        }
        ...
        if (name.EndsWith("__offset-fetch", StringComparison.Ordinal))
        {
            ...
            Assert.Contains(" TO ", command.Sql, StringComparison.OrdinalIgnoreCase);
            Assert.Contains(
                command.Parameters,
