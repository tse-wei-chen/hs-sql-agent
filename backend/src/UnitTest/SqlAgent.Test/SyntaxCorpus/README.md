...
- `sql-generated-postgres-quoted-function-compatibility-floor.json` retains the PostgreSQL quote-sensitive Cartesian floor and now also covers provider-native quoted function identity on all six dialects plus qualified native function identity on PostgreSQL, MySQL, SQL Server, Oracle, and Firebird; SQLite qualification remains an explicit negative grammar contract.
- `sql-generated-postgres-distinct-from-compatibility-floor.json` is the cross-dialect null-safe comparison expansion floor: PostgreSQL/SQLite/Firebird native `IS [NOT] DISTINCT FROM`, MySQL `<=>`, SQL Server 2022+ native syntax, and proven Oracle lowering.
- `sql-generated-cast-type-compatibility-floor.json` is the 123-case six-provider CAST/type floor covering same-provider type grammar, PostgreSQL postfix casts, and proven cross-provider type lowering.
...
| Native/quote-sensitive function identifier matrices | 31 |
| Cross-dialect null-safe comparison matrix | 60 |
| Six-provider CAST/type matrix | 123 |
| Oracle 12.1 native row-limiting matrix | 72 |
| MySQL session-mode matrix | 48 |
| SQL Server version/compatibility matrix | 36 |
| **Total positive query floor** | **5055** |

The CAST/type matrix adds 99 same-provider cases across PostgreSQL, MySQL, SQL Server, SQLite, Oracle, and Firebird, 15 PostgreSQL postfix-cast cases, and 9 proven cross-provider lowering cases. Parser-native CAST syntax is classified into the closed `SqlType` algebraic model before normalization; provider-native types retain provider identity, and cross-provider lowering remains fail-closed when no semantic mapping is proven. The recursive matrix additionally cross-products four proven native providers, four legal recursive-member shapes, four root consumers, and root ordering, with explicit source/target server-version proofs. The PostgreSQL native modifier matrix cross-products root/CTE placement, DISTINCT ON, CROSS/LEFT LATERAL, NULLS FIRST/LAST, and FETCH WITH TIES under an explicit PostgreSQL 13 source/target profile. The function-identifier floor keeps the original PostgreSQL Cartesian 5×4 quote-sensitive matrix and adds six provider-native quoted forms plus five provider-native qualified forms. Quote intent and qualification are separate proofs: legal source delimiters are enforced by each dialect lexer, SQLite scalar-function qualification remains rejected, and quoted/qualified native identity is preserved only for same-provider compilation so database/schema/package names are never silently reinterpreted across providers. The null-safe comparison contract keeps a single `DistinctFrom` / `NotDistinctFrom` semantic in the closed F# AST. PostgreSQL, SQLite, and Firebird use native `IS [NOT] DISTINCT FROM`; MySQL native `<=>` canonicalizes to `NotDistinctFrom` and distinctness renders as `NOT (<=>)`; SQL Server 2022+ is version-gated to its native predicate; Oracle uses a CASE-based lowering only for repeatable scalar operands so volatile functions, windows, and subqueries are never duplicated. Raw source spelling remains dialect-correct rather than pretending every provider accepts PostgreSQL syntax. Native quoted or qualified function identities stay opaque on their legal source provider so case-sensitive names and provider namespaces cannot be reinterpreted as portable built-ins; cross-provider targets must fail at typed capability boundaries rather than losing quote or qualification intent during normalization. The Oracle native matrix cross-products root/CTE placement, simple/join/correlated sources, explicit NULL ordering, row-count versus percentage FETCH, and ONLY versus WITH TIES under an explicit Oracle 12.1 source/target profile. The profile-sensitive matrix makes source/target session modes and compatibility levels first-class parity inputs. MySQL cross-products concat/ANSI-quote modes with root, CTE, scalar-subquery, predicate, and ordering contexts; SQL Server cross-products ordered `STRING_AGG` across root, CTE, scalar-subquery, and grouped contexts under explicit ServerVersion 14.0 and CompatibilityLevel 110 proofs. The matrices exercise parsing, query facts/binding, validation, compilation, and native rendering. Assertions are semantic and renderer-aware: source spelling is not required to survive when a target renderer intentionally lowers it to an equivalent native form.
...
Generated mutation tests additionally place malformed or wrong-dialect syntax in nested positions such as CTE bodies, scalar subqueries, and set branches. Quoted-function negative matrices keep SQLite qualified scalar-function syntax fail-closed, retain twenty cross-provider target-capability rejections across the PostgreSQL positive identifier/context matrix, and keep four semantic fail-closed cases for native quoted functions carrying unmodeled DISTINCT, aggregate-local ORDER BY, FILTER, or OVER modifiers. Cross-provider qualified native identities are likewise rejected at the typed target-capability boundary. Null-safe comparison source negatives also reject MySQL `<=>` spelling on each of the other five dialects at the typed source-capability boundary, symmetric with the dialect gate for `IS [NOT] DISTINCT FROM`. Recursive CTE mutations add 23 typed-binding cases covering anchor self-reference, duplicate/nested-only self-reference, invalid recursive set shape, non-PostgreSQL portable-subset violations, and Firebird's UNION ALL requirement. DML mutation tests cover parser failures, source-capability failures, and policy denial.
